...
import { path, fse } from '../../../core-imports';
// Returns a temporary file
// Example: for /some/file will return /some/.file.tmp
function getTempFilename(file: string | URL) {
  const f =
    file instanceof URL
      ? file.pathname // 👈 key change
      : file.toString();
...
export class Writer {
  ...
  async __write(data) {
    ...
    try {
      // Atomic write
      await fse.writeFile(this.__tempFilename, data, 'utf-8');
      await retryAsyncOperation(
        async () => {
          await fse.rename(this.__tempFilename, this.__filename);
        },
        10,
        100,
      );
