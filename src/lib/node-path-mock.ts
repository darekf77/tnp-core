...
export const path = {
  ...
  basename: function basename(pathArg, ext) {
    ...
    if (ext !== undefined && ext.length > 0 && ext.length <= pathArg.length) {
      ...
      if (start === end) end = firstNonSlashEnd; else if (end === -1) end = pathArg.length;
  ...
  parse: function parse(pathArg) {
    ...
    if (startDot === -1 || end === -1 ||
      ...
      preDotState === 1 && startDot === end - 1 && startDot === startPart + 1) {
      if (end !== -1) {
        if (startPart === 0 && isAbsolute) ret.base = ret.name = pathArg.slice(1, end); else ret.base = ret.name = pathArg.slice(startPart, end);
      }
    } else {
    ...
    if (startPart > 0) ret.dir = pathArg.slice(0, startPart - 1); else if (isAbsolute) ret.dir = '/';
